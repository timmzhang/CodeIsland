...
final class AppStateToolUseCacheTests: XCTestCase {
    ...
    /// each other.
    /// Merge guard (#216 vs #147): the orphan drain must stand down as soon as the
    /// activity event carries a tool_use_id, even though the queued card has none.
    /// Such an event names one specific tool call, so correlation — not a blanket
    /// auto-allow — owns it.
    func testOrphanDrainSkippedWhenActivityEventCarriesToolUseId() async throws {
        let appState = AppState()
        ...
        // A different parallel call completes, and it DOES carry an id.
        appState.handleEvent(try makeHookEvent(
            name: "PostToolUse",
            sessionId: "s1",
            toolName: "Bash",
